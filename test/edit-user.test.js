...
describe('======= test for user change avatar =======', function() {
  ...
  describe('Edit user avatar direct to database', function() {
      it('expect to return new edited user avatar', function(done) {
          user.create({username: 'user_testing_for_change_avatar', email: 'user@testing.com', password: crypto.createHash('md5').update('userpassword').digest("hex"), avatar: 'http://dummy-avatar-image.com'}).then((data) => {
              user.update({
                  avatar: 'http://edited-dummy-avatar-image.com'
              }, {
                  fields: ['avatar'],
                  where: {
                      id: data.id
                  ...
                  user.findOne({
                      ...
                      expect(user_data.avatar).to.be.equal('http://edited-dummy-avatar-image.com')
                      done()
                  })
              })
          })
  ...
  describe('Try to Edit user avatar without login via route "/auth/edit_avatar"', function() {
      it('Expect to return status 401 (Unauthorized user)', function(done) {
          user.create({username: 'user_testing_for_change_avatar_via_api', email: 'user_via_api@testing.com', password: crypto.createHash('md5').update('userpassword').digest("hex"), avatar: 'http://dummy-avatar-image.com'}).then((data) => {
              chai.request(urlApi).put('/auth/edit_avatar/' + data.id).send({avatar: 'http://edited-dummy-avatar-image.com'}).end(function(req, res) {
                  expect(401).to.be.equal(res.status)
                  done()
              })
          })
  ...
  describe('Edit user avatar after login via route "/auth/edit_avatar"', function() {
      it('Expect to return status 200', function(done) {
          user.create({username: 'authorized_user_for_change_avatar_via_api', email: 'authorized_user@testing.com', password: crypto.createHash('md5').update('authorizeduserpassword').digest("hex"), avatar: 'http://dummy-avatar-image.com'}).then((data) => {
              chai.request(urlApi).post('/auth/login').send({username: data.username, password: 'authorizeduserpassword'}).end((err, res) => {
                  const userDecoded = decode(res.body)
                  user.findOne({
                      where: {
                      ...
                      chai.request(urlApi).put('/auth/edit_avatar/' + one_user.id).send({avatar: 'http://edited-dummy-avatar-image.com'}).set({authorization: `Bearer ${res.body}`}).end(function(req, res) {
                          expect(200).to.be.equal(res.status)
                      })
                  })
                  done()
